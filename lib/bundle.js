...
const Howl = __webpack_require__(7);
...
class Board {
  ...
  checkSwap(){
    if(this.isValidPos()){
      ...
      if(this.findHorizontalMatch().concat(this.findVerticalMatch()).length > 0){
        this.removeCell(true);
        setTimeout(() => {this.removeCell();}, 1000);
        this.updateScore();
        this.updateMoves();
      } else {
        this.swapCells();
  ...
  timer(){
    let duration = TIME * 60, minutes, seconds;
    console.log("timer")
      setInterval(() => {
